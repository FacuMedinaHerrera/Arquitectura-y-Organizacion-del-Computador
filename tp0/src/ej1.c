...
{
	char* producto=productos[id];
	
	if(moneda==USD){
		fprintf(pfile,"%s, %d, USD, %d\n",producto,precio,stock);
	}else{
		fprintf(pfile,"%s, %d, ARG, %d\n",producto,precio,stock);
	}
...
void procesar_e_imprimir_catalogo(FILE *pfile)
{
	...
	//para obterner los primeros 8 bits que se corresponden
	//al ID, hay que shiftear el dato y dejar solo esos 8 bits
	//luego con una mascara se obtienen los bits requeridos

	for(uint32_t i=0;i<32;i++){
		uint32_t ID1=catalogo[i];
		uint32_t ID2=catalogo[i];
		uint32_t precio1=catalogo[i];
		uint32_t moneda1=catalogo[i];
		uint32_t auxPrecio2=catalogo[i];//quedaran los ultimos 3 bits
		uint32_t aux2Precio2=catalogo[i+1];
		uint32_t moneda2=catalogo[i+1];
		uint32_t stock1=catalogo[i+1];
		uint32_t stock2=catalogo[i+1];

		ID1=(ID1>>24)&0xFF;//me quedan los 8 bits del dato
		ID2=(ID2>>16)&0xFF;
		precio1=(precio1>>4)&0xFFF;
		moneda1=(moneda1>>3)&0x1;
		auxPrecio2=auxPrecio2&0x7;//0111
		aux2Precio2=aux2Precio2>>23;
		uint32_t precio2=(auxPrecio2<<9)|aux2Precio2;
		moneda2=(moneda2>>22)&0x1;
		stock1=(stock1>>11)&0x7FF;//0111 1111 1111
		stock2=stock2&0x7FF;
