...
void ej1(
	...
	for(uint32_t fila=0;fila<height;fila++){
		for (uint32_t columna =0;columna<width;columna++){
			uint32_t index=fila*width+columna; 
			rgba_t pixel=src[index];
			float luminosidad=0.2126*pixel.r+pixel.g*0.7152+0.0722*pixel.b;
			pixel.a=255;
			pixel.r=luminosidad;
			pixel.g=luminosidad;
			pixel.b=luminosidad;
